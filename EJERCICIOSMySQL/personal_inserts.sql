...
#-----------------------------------------------
SELECT * FROM empleados LEFT JOIN departamentos on empleados.id_depto = departamentos.id_depto;
#2. Obtener los datos completos de los departamentos.
SELECT * FROM departamentos;
#3. Listar el nombre de los departamentos.
SELECT ciudad FROM departamentos;
...
SELECT (sal_emp+comision_emp+500 ) AS 'TOTAL',id_depto,nombre FROM empleados WHERE id_depto = 3000 ORDER BY nombre ASC;
#12 Muestra los empleados cuyo nombre empiece con la letra J
SELECT e.nombre FROM empleados e WHERE nombre LIKE 'J%';
...
SELECT e.nombre,(e.sal_emp + e.comision_emp) AS 'SALARIO TOTAL' FROM empleados e WHERE comision_emp > 1000;
#14 Obtener un listado similar al anterior, pero de aquellos empleados que NO tienen comisión.
SELECT e.nombre,(e.sal_emp + e.comision_emp) AS 'SALARIO TOTAL' FROM empleados e WHERE comision_emp = 0;
...
SELECT nombre FROM empleados WHERE NOT nombre LIKE '%MA%';
...
SELECT MAX(nombre) AS 'ULTIMO NOMBRE' FROM empleados;
#22 Hallar el salario más alto, el más bajo y la diferencia entre ellos.
SELECT MAX(sal_emp) AS 'SALARIO MAXIMO', MIN(sal_emp) AS 'SALARIO MINIMO', (MAX(sal_emp) - MIN(sal_emp)) AS 'DIFERENCIA'
FROM empleados;
#23 Hallar el salario promedio por departamento.
SELECT id_depto,AVG(sal_emp) AS 'PROMEDIO' 
FROM empleados
GROUP BY id_depto;
#24 HAVING Hallar los departamentos que tienen más de tres empleados. Mostrar el número de empleados de esos departamentos.
select d.id_depto as 'ID DEPTO', d.nombre_depto as 'Nombre DPTO', count(*) as 'Empleadoss'
...
SELECT e.nombre,e.cargo_emp,e.cod_jefe as 'CODIGO', count(*) 'total emp'
...
SELECT e.nombre,e.sal_emp From empleados e
